...
function createTaskPoller({
    ...
    function doPoll() {
        ...
        function fetchNextPage() {
            ...
            longPollRequest = swfClient[method](actualParams);

            longPollRequest.on('success', ({ data }) => {
                // Request is no longer cancelable
                longPollRequest = null;

                // handle incoming page of data
                if (!data.taskToken) {
                    // Missing taskToken means long polling timed out and we need to start over.
            ...
            longPollRequest.on('error', (err) => {
                // Request is no longer cancelable
                longPollRequest = null;

                if (err.code !== 'RequestAbortedError') {
                    // This is the error generated by calling longPollRequest.abort()
